...
var showAnswer = function(answer) {
	...
	var answerPic = $('.prof-pic a');
	answerPic.attr('href', answer.user.link);
	var image = result.find('.ans_image');
	image.attr('src', answer.user.profile_image);
	console.log(image);

	var rep = result.find('.rep');
	rep.text(answer.user.reputation);

	var post = result.find('.post');
	post.text(answer.post_count);
	return result;
};
...
var getAnswerers = function (query) {
	var request = {
		site: 'stackoverflow',
	};
